...
{
    public class SettingAddChannelCache : ISettingPage
    {
        public string Name { get; set; } = "cache_add_channel";

        public async void Display(YukiModule Module, YukiCommandContext Context)
        {
            await Module.ReplyAsync(Module.Language.GetString("setting_channel_add_cache_desc"));
        }
        ...
        public async Task Run(YukiModule Module, YukiCommandContext Context)
        {
            ...
            if (result.IsSuccess)
            {
                if (MentionUtils.TryParseChannel(result.Value.Content, out ulong channelId))
                {
                    ...
                    await Module.ReplyAsync(Module.Language.GetString("cache_added") + ": " + $"<#{channelId}>");
                }
            }
        }
    }
