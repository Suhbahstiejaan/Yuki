...
{
    public class SettingAddModeratorRole : ISettingPage
    {
        public string Name { get; set; } = "roles_moderator_add";

        public async void Display(YukiModule Module, YukiCommandContext Context)
        {
            await Module.ReplyAsync(Module.Language.GetString("setting_moderator_add_role_desc"));
        }
        ...
        public async Task Run(YukiModule Module, YukiCommandContext Context)
        {
            ...
            if (result.IsSuccess)
            {
                if (MentionUtils.TryParseRole(result.Value.Content, out ulong roleId))
                {
                    ...
                    await Module.ReplyAsync(Module.Language.GetString("moderator_added") + ": " + Context.Guild.GetRole(roleId).Name);
                }
            }
        }
    }
